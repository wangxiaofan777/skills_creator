...
class ApiError(Exception):
    def __init__(self, kind, detail=""):
        ...
        self.kind = kind  # no_auth / no_permission / not_found / unreachable / http_error
        self.detail = detail
...
def api_get(cfg: dict, path: str, params: dict | None = None) -> dict:
    ...
    if cfg.get("token"):
        # SonarQube token 认证: token 作为用户名, 密码为空
...
def cmd_issues(cfg: dict, files: list, repo: str) -> dict:
    ...
    return {"total": len(issues), "bySeverity": summary, "issues": issues}


def cmd_rules(cfg: dict, langs: list) -> dict:
    """列出项目各语言质量配置中的活跃规则(摘要,按严重级排序)。"""
