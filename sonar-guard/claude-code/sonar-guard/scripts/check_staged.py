...
# 本地启发式规则: 只收录误报率极低的模式 (rule_id, 语言集合, 正则, 说明, 严重级)
LOCAL_RULES = [
        ...
        for rule_id, langs, pattern, msg, sev in LOCAL_RULES:
            ...
            for i, line in enumerate(lines, 1):
                ...
                if pattern.search(line):
                    ...
                    break  # 同一文件同一规则只报第一处, 控制噪音
    return findings


def fetch_server_issues(repo, cfg, files):
    """查询暂存文件在服务器上的未解决 issue; 任何异常都静默降级。"""
    try:
        ...
        return issues, "ok"
    except Exception:  # noqa: BLE001 钩子里任何错误都不能让提交流程崩溃
...
def main():
    ...
    if blocking:
        ...
        print("可让 AI 助手运行 sonar-guard 技能帮你分析和修复这些问题。")
        return 1
