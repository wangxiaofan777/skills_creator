...
When `install.py` installs the pre-commit hook for a target repository, it SHALL copy `sonar_api.py`, `check_staged.py`, and `scan.py` into both `.git/hooks/sonarguard/` and a repository-root `.sonarguard/` directory, and SHALL write `.sonarguard/.installed-by-sonar-guard` to mark managed installs.
...
#### Scenario: User in metis without install
